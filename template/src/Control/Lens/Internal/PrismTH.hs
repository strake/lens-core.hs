...
stabToType stab@(Stab cx ty s t a b) = ForallT vs cx $
  case ty of
    PrismType  | stabSimple stab -> ''Lens.Prism'  `conAppsT` [t,b]
               | otherwise       -> ''Lens.Prism   `conAppsT` [s,t,a,b]
    ReviewType                   -> ''Lens.Review  `conAppsT` [t,b]
...
#ifndef HLINT
         ty | Map.null sub = appsT (conT ''Lens.Iso') [t,b]
            | otherwise    = appsT (conT ''Lens.Iso) [s,t,a,b]
#endif
...
makeConPrismExp ::
  ...
  ExpQ
makeConPrismExp stab cons con = appsE [varE 'Lens.prism, reviewer, remitter]
  where
...
makeConIsoExp con = appsE [varE 'Lens.iso, remitter, reviewer]
  where
  conName = view nconName con
...
makeConReviewExp con = appE (varE 'Lens.unto) reviewer
  where
  conName = view nconName con
  fields  = length (view nconTypes con)
...
makeClassyPrismClass t className methodName cons =
  do r <- newName "r"
#ifndef HLINT
     let methodType = appsT (conT ''Lens.Prism') [varT r,return t]
#endif
